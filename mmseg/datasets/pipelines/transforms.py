...
from mmseg.utils.multiple_files import split_images
...
class Resize(object):
    ...
    def __init__(self,
        ...
        if img_scale is None:
            ...
            if isinstance(img_scale, list):
                if mmcv.is_list_of(img_scale, tuple):
                    self.img_scale = img_scale
                elif mmcv.is_list_of(img_scale, list):
...
@PIPELINES.register_module()
class RandomCropDisplacement(RandomCrop):
    def __init__(self, displacement_sigma, crop_size, cat_max_ratio=1., ignore_index=255):
        super(RandomCropDisplacement, self).__init__(
            crop_size=crop_size,
    ...
    def displace_crop_bbox(self, crop_bbox, h, w):
        dx, dy = np.random.normal(loc=0, scale=self.displacement_sigma, size=2)
        dx, dy = int(np.round(dx, decimals=0)), int(np.round(dx, decimals=0))
    ...
    def __call__(self, results):
        ...
        # crop the image
        img1, img2 = np.split(img, indices_or_sections=2, axis=2)
        img1 = self.crop(img1, self.displace_crop_bbox(crop_bbox, img.shape[0], img.shape[1]))
